...
- Current focus: continue S5 Day 3/Day 4, especially frontend round-trip coverage, TS `strict:true`, OpenAPI/Postman, and ErrorBoundary. Note: `make worker` needs LLM config to boot (`LLM_PROVIDER=mock` for export-only smoke); Makefile go-run targets fixed to package paths.
