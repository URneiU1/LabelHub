  ...
  if (aiReview) {
    ...
    return (
      <section className="ai-section" aria-label="AI 预审结论">
        ...
        <div className="hr-rerun__scores">AI {verdict} · {score}</div>
        <div style={{ color: verdict === 'pass' ? 'var(--lh-success)' : 'var(--lh-danger)', fontWeight: 700, fontSize: 13 }}>verdict: {verdict}</div>
        <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 10 }}>score: {score}</div>
        {dimensions.length > 0 ? (
        ...
        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <span className="lh-muted">tokens: {aiReview.tokensInput + aiReview.tokensOutput}</span>
          <span className="lh-muted">latency: {aiReview.latencyMs}ms</span>
        </div>
      </section>
    )
  }
  ...
  if (submission?.aiVerdict) {
    ...
    return (
      <section className="ai-section" aria-label="AI 预审结论">
        ...
        <div style={{ fontWeight: 700, fontSize: 13 }}>score: {score}</div>
      </section>
    )
  }
