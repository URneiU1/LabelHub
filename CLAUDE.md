...
- Current focus: start S6 by writing `docs/PLAN-S6-IMPL.md` first, then implement performance/observability work against explicit verification gates. Note: `make worker` needs LLM config to boot (`LLM_PROVIDER=mock` for export-only smoke); Makefile go-run targets fixed to package paths.
