...
- AI 预审 P1 安全/状态/前端竞态问题已收敛,并补了 P2 action stale guard/provider error/non-retryable validation cleanup;golden sample 后端 persistence API、Owner 管理 UI、batch result table、task-scoped dry-run history API、Owner history/trend 最小视图、Reviewer AI verdict/score/detail/audit 展示、Reviewer failed/dead AI retry、Reviewer 规则选择/Owner 编辑跳转入口、单样本 queued dry-run + polling、同步串行 batch dry-run endpoint、基础 batch delay 配置、provider 429/backoff 短重试、server-side dry-run quota/circuit breaker、AI worker 5xx 熔断、AI 自动 approved、Labeler 3s 自动保存/修订入口和 Reviewer 批量操作已具备。剩余 demo 风险是单样本异步 dry-run 仍是 API 进程内 goroutine,不是 durable Asynq worker;Reviewer 侧不直接修改/切换 active prompt,更丰富的 trend/history 分析也还未做。
...
- 推进 S3 下一段:如需上台更稳,把 single golden dry-run 从 API goroutine 升级为 durable Asynq job,并把 quota/circuit breaker 状态展示到 Owner UI;再评估 Reviewer 是否需要直接发起 active rule switch。
