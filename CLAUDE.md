...
- Current focus: continue S5 Day 5, especially production compose/Caddy, `make dev`, ARCHITECTURE/DEPLOY docs, and S5 acceptance docs. Note: `make worker` needs LLM config to boot (`LLM_PROVIDER=mock` for export-only smoke); Makefile go-run targets fixed to package paths.
