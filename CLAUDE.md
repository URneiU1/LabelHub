...
- Current focus: execute `docs/PLAN-S6-IMPL.md`, starting with Editorial tokens and shared status/loading/empty primitives before module layout changes. Note: `make worker` needs LLM config to boot (`LLM_PROVIDER=mock` for export-only smoke); Makefile go-run targets fixed to package paths.
