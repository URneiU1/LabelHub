...
- Current branch: **`s5-engineering-quality`**. S5 engineering-quality sprint has started from `docs/PLAN-S5-IMPL.md`: CI now includes `./pkg/exporter`; state machine transition coverage is 100%; `review.Apply` and outbox publisher tests were expanded; AI worker verdict mapping and circuit-open failover are covered; exporter row-load DB errors are retryable instead of terminal failed; stats dimension averages only count the current submission revision; StatsBoard retry uses request sequence guarding. Targeted coverage gates passed for statemachine/review/outbox/handler/exporter/worker and targeted `pnpm -F web test -- StatsBoard` passed.
...
- Current focus: continue S5 Day 2, especially testcontainers main-flow integration. Note: `make worker` needs LLM config to boot (`LLM_PROVIDER=mock` for export-only smoke); Makefile go-run targets fixed to package paths.
