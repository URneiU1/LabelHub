...
func TestAIDryRunsListIncludesGuardStatus(t *testing.T) {
	t.Setenv("LLM_DRY_RUN_QUOTA_MAX_RUNS", "5")
	t.Setenv("LLM_DRY_RUN_CIRCUIT_MAX_FAILURES", "2")
	t.Setenv("LLM_DRY_RUN_GUARD_WINDOW_MINUTES", "30")
	db, mock, sqlDB := newMockDB(t)
	defer sqlDB.Close()
	...
	mock.ExpectQuery(`(?is)^SELECT.+FROM .tasks.`).
		WillReturnRows(sqlmock.NewRows([]string{"id", "owner_id", "title", "status"}).
			AddRow(1, 7, "Task", "draft"))
	mock.ExpectQuery(`(?is)^SELECT.+FROM .ai_dry_runs.+ORDER BY created_at DESC, id DESC.+LIMIT`).
		WillReturnRows(sqlmock.NewRows([]string{
			"id", "task_id", "ai_prompt_id", "golden_sample_id", "prompt_version",
			"payload_snapshot", "expected_answer_snapshot", "expected_verdict", "actual_verdict", "matched_expected",
			"status", "result", "error_msg", "created_by", "created_at", "finished_at",
		}))
	mock.ExpectQuery(`(?is)^SELECT count\(\*\) FROM .ai_dry_runs. WHERE task_id = .+created_at >=`).
		WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(4))
	mock.ExpectQuery(`(?is)^SELECT count\(\*\) FROM .ai_dry_runs. WHERE task_id = .+status = .+created_at >=`).
	...
	data := responseData(t, rec)
	guard := data["guard"].(map[string]any)
	if guard["windowMinutes"] != float64(30) || guard["quotaRemaining"] != float64(1) || guard["circuitOpen"] != true {
		t.Fatalf("unexpected guard status: %v", guard)
	}
	if guard["state"] != "circuit_open" {
		t.Fatalf("guard state = %v, want circuit_open", guard["state"])
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatalf("expectations not met: %v", err)
	}
