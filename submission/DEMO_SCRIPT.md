...
#### 2.1 切角色 + 领取(3:00 - 3:15) · 15s
- 退出 → 登 `labeler1` / `123456`
- 进入 **任务广场** → 在 `qa_quality` 上点 **「领取任务」**(first_come = 整体独占领取整个大任务)
- **再**点左栏 **「标注工作台」** → 自动恢复刚领的任务、进入三列作答页

> ⚠️ **顺序别错:必须先在任务广场「领取任务」,再进「标注工作台」**。没领取就直接点工作台 → 因为没有"进行中的任务",会(按设计)回退到任务广场,不是 bug。

> "Labeler 是标注员。任务广场按 first-come 抢占,我领取 qa_quality 整个任务,进标注工作台开始标第一题。"
