...
- S2 Designer editing v1 is in place: owners can browse task template versions, edit latest templates with click-to-append widgets, delete fields, update simple properties, save as a new version, and fork historical readonly versions without leaking `_draftId`.
- S2 Designer now fails closed when the route task ID does not match the loaded template's task ID, preventing save/fork into the wrong task after manual URL edits or bad future navigation.
...
- Continue Designer implementation with ordering/copy, per-field validation display, real item preview, or Tabs/Group bonus widgets.
