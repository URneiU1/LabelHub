...
- Current branch: **`s5-engineering-quality`**. S5 engineering-quality sprint has started from `docs/PLAN-S5-IMPL.md`: CI now includes `./pkg/exporter` and a separate `-tags=integration` job; state machine transition coverage is 100%; `review.Apply` and outbox publisher tests were expanded; AI worker verdict mapping and circuit-open failover are covered; testcontainers integration now covers submit→outbox/Redis→AI result→revise→resubmit→approve→export against real MySQL+Redis; web `strict:true` is enabled and Renderer/Designer round-trip tests cover ShowItem modes, LLMTrigger, and Designer payload→Renderer parsing; exporter row-load DB errors are retryable instead of terminal failed; stats dimension averages only count the current submission revision; StatsBoard retry uses request sequence guarding. Targeted coverage gates passed for statemachine/review/outbox/handler/exporter/worker, integration passed locally with Colima env, `pnpm -F web build/lint` passed, and targeted web tests passed.
...
- Current focus: continue S5 Day 4/Day 5, especially OpenAPI/Postman, ErrorBoundary, production compose/Caddy, `make dev`, and S5 acceptance docs. Note: `make worker` needs LLM config to boot (`LLM_PROVIDER=mock` for export-only smoke); Makefile go-run targets fixed to package paths.
