...
- Current focus: continue `docs/PLAN-S6-IMPL.md` Day3 after Day1 primitives and Day2 status/empty/loading replacement landed. Owner Dashboard, ExportPanel, Labeler Plaza, and Reviewer Queue now reuse shared `StatusBadge`/`EmptyState`/`LoadingBlock` in their main status/loading/empty surfaces. Next is Designer responsive layout plus real Tabs/Group interaction. Note: `make worker` needs LLM config to boot (`LLM_PROVIDER=mock` for export-only smoke); Makefile go-run targets fixed to package paths.
